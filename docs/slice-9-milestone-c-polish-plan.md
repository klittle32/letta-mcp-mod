...
- `auth-clear` through proxy and `/toolbox` command routing;
- guarded Letta UI status values and short-lived command panels;
- visible MCP UI resource URIs in list/search/describe and `/toolbox tools`;
...
- Update `/toolbox tools` only if command-level regex UX is added; otherwise keep regex support model-facing through the proxy tool.
...
2. `/toolbox auth-clear remote` parses and routes if we choose to expose the command alias.
...
3. Add parser support in `src/features/mcp-command.ts` if adding `/toolbox auth-clear <server>`.
...
2. `/toolbox reconnect <server>` opens/updates/closes a short panel when panels are available.
3. `/toolbox auth-start <server>` can show “OAuth URL generated” panel, but long URL remains in command output.
...
- Minimal first pass:
  ...
  - add `/toolbox resource <server> <uri>` or proxy action only if it fits existing mode precedence cleanly.
